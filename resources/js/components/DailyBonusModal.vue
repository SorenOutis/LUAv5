...
<template>
    <Dialog :open="isOpen" @update:open="handleClose">
        <DialogContent class="max-w-sm border-0 shadow-2xl overflow-hidden">
            ...
            <div :class="isDarkMode ? 'from-amber-900/30 to-orange-900/30' : 'from-amber-200 to-orange-200'" class="absolute -top-20 -right-20 w-40 h-40 bg-gradient-to-br rounded-full blur-3xl opacity-30"></div>
            <div :class="isDarkMode ? 'from-yellow-900/30 to-amber-900/30' : 'from-yellow-200 to-amber-200'" class="absolute -bottom-20 -left-20 w-40 h-40 bg-gradient-to-tr rounded-full blur-3xl opacity-30"></div>
            ...
            <div class="relative z-10 flex flex-col items-center gap-6 py-8 px-6">
                ...
                >
                    <div v-if="claimSuccess" class="flex flex-col items-center gap-4 w-full">
                        ...
                        <div class="text-center">
                            ...
                            <p :class="isDarkMode ? 'text-gray-300' : 'text-gray-600'" class="text-sm">
                                You earned <span class="font-semibold text-amber-500">{{ xpAmount }} XP</span>
                            </p>
                            <p :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'" class="text-xs mt-2">
                    ...
                    <div v-else-if="!hasReceivedToday" class="flex flex-col items-center gap-4 w-full">
                        ...
                        <!-- XP Display -->
                         <div class="relative mt-2">
                             ...
                             <div :class="isDarkMode ? 'bg-gradient-to-br from-slate-800 to-slate-700 border-slate-600/60' : 'bg-gradient-to-br from-white to-amber-50 border-amber-200/60'" class="relative rounded-2xl px-8 py-6 border shadow-lg">
                                 <div class="flex items-center justify-center gap-2">
                                     <span class="text-5xl font-bold bg-gradient-to-r from-amber-500 to-orange-500 bg-clip-text text-transparent">
                                         +{{ xpAmount }}
                                     </span>
                                     <span :class="isDarkMode ? 'text-gray-200' : 'text-gray-700'" class="text-2xl font-semibold">XP</span>
                                 </div>
                             </div>
                         </div>
                    </div>
                ...
                >
                    <div v-if="claimError" :class="isDarkMode ? 'bg-gradient-to-r from-red-900/30 to-rose-900/30 border-red-700/60' : 'bg-gradient-to-r from-red-50 to-rose-50 border-red-200/60'" class="w-full border rounded-xl p-4 shadow-sm">
                         <div class="flex items-start gap-3">
                             <X class="w-5 h-5 text-red-600 flex-shrink-0 mt-0.5" />
                             <div>
                                 <p :class="isDarkMode ? 'text-red-400' : 'text-red-700'" class="font-semibold text-sm">{{ claimError }}</p>
                             </div>
                         </div>
                     </div>
                </transition>
                ...
                <div class="w-full flex gap-3 mt-2">
                    ...
                    <Button
                         ...
                         class="flex-1 font-semibold py-2.5 rounded-lg border shadow-sm hover:shadow-md transition-all duration-300 disabled:opacity-75"
                     >
                ...
                <!-- Streak Reminder -->
                 <div :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'" class="text-center text-xs mt-2">
